...
function findKey(row, ...keys) {
  for (const k of keys) {
    ...
    if (found && row[found] !== "") return parseFloat(row[found]) || row[found];
  }
  return null;
}
...
export default function App() {
  ...
  const campaigns = {};
  data.forEach(row => {
    const name = findKey(row, "campana", "campaign", "campaña") || "Sin nombre";
    if (!campaigns[name]) campaigns[name] = { name, cpl: [], ctr: [], cpc: [], frecuencia: [], gasto: [], conversiones: [], dias: 0 };
    campaigns[name].dias++;
    const cpl = findKey(row, "cpl"); if (cpl) campaigns[name].cpl.push(cpl);
    const ctr = findKey(row, "ctr"); if (ctr) campaigns[name].ctr.push(ctr);
    const cpc = findKey(row, "cpc"); if (cpc) campaigns[name].cpc.push(cpc);
    const frec = findKey(row, "frecuencia", "frequency"); if (frec) campaigns[name].frecuencia.push(frec);
    const gasto = findKey(row, "gasto", "spend"); if (gasto) campaigns[name].gasto.push(gasto);
    const conv = findKey(row, "conversiones", "conversions"); if (conv) campaigns[name].conversiones.push(conv);
  });
  ...
  const avgCPL = campList.filter(c => c.cpl).reduce((a, c, _, arr) => a + c.cpl / arr.length, 0);
  const alertas = campList.filter(c => c.frecuencia > 4.5 || c.cpl > 1.5 || c.ctr < 1);
    ...
    <div style={{ fontFamily: "'DM Mono', monospace", background: "#050508", minHeight: "100vh", color: "#e2e8f0" }}>
      <style>{`
        ...
        * { box-sizing: border-box; margin: 0; padding: 0; }
        ...
        .row-card { padding: 16px 20px; background: #0d0d14; border: 1px solid #1a1a2e; border-radius: 10px; transition: all 0.15s; }
        .row-card:hover { border-color: #2d2d50; background: #0f0f1a; }
        .metric-val { font-family: 'Bebas Neue', sans-serif; letter-spacing: 1px; }
        .alert-badge { padding: 2px 8px; border-radius: 20px; font-size: 10px; font-weight: 600; letter-spacing: 1px; }
      `}</style>
      ...
      <div style={{ padding: "16px 28px", borderBottom: "1px solid #0f0f1f", display: "flex", alignItems: "center", justifyContent: "space-between" }}>
        ...
        <div style={{ display: "flex", alignItems: "center", gap: 12 }}>
          {alertas.length > 0 && (
            <div style={{ background: "#1c0a0a", border: "1px solid #7f1d1d", color: "#fca5a5", borderRadius: 8, padding: "6px 12px", fontSize: 11 }}>
      ...
      {loading ? (
        ...
        <div style={{ padding: "24px 28px", maxWidth: 1200, margin: "0 auto" }}>
          ...
          <div style={{ display: "grid", gridTemplateColumns: "repeat(4, 1fr)", gap: 14, marginBottom: 24 }} className="fade">
            {[
              { label: "GASTO TOTAL", val: `$${totalGasto.toFixed(0)}`, sub: `${data.length} registros`, color: "#3b82f6", icon: "💰" },
              { label: "CONVERSIONES", val: totalConv || "—", sub: "total acumulado", color: "#22c55e", icon: "🎯" },
              { label: "CPL PROMEDIO", val: avgCPL ? `$${avgCPL.toFixed(2)}` : "—", sub: avgCPL <= 1 ? "✓ Bajo objetivo" : "⚠ Sobre objetivo", color: avgCPL <= 1 ? "#22c55e" : "#f59e0b", icon: "📉" },
              ...
              <div key={i} className="card">
                ...
                <div className="metric-val" style={{ fontSize: 32, color: k.color }}>{k.val}</div>
          ...
          {tab === "campaigns" && (
            <div className="fade" style={{ display: "flex", flexDirection: "column", gap: 10 }}>
              {campList.length === 0 ? (
                ...
                const hasAlert = c.frecuencia > 4.5 || c.cpl > 1.5 || c.ctr < 1;
                return (
                  <div key={i} className="row-card">
                    <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", marginBottom: 14 }}>
                      <div style={{ display: "flex", alignItems: "center", gap: 10 }}>
                        ...
                        {hasAlert && <span className="alert-badge" style={{ background: "#1c0a0a", color: "#fca5a5", border: "1px solid #7f1d1d" }}>⚠ ALERTA</span>}
                      </div>
                      <div style={{ fontSize: 10, color: "#374151", letterSpacing: 1 }}>{c.dias} DÍA{c.dias !== 1 ? "S" : ""}</div>
                    </div>
                    <div style={{ display: "grid", gridTemplateColumns: "repeat(6, 1fr)", gap: 16 }}>
                      {[
                        { label: "CPL", val: c.cpl ? `$${c.cpl.toFixed(2)}` : "—", warn: c.cpl > 1, bar: c.cpl, max: 5, color: c.cpl > 1 ? "#ef4444" : "#22c55e" },
                        { label: "CTR", val: c.ctr ? `${c.ctr.toFixed(2)}%` : "—", warn: c.ctr < 1, bar: c.ctr, max: 10, color: c.ctr < 1 ? "#f59e0b" : "#3b82f6" },
                        { label: "CPC", val: c.cpc ? `$${c.cpc.toFixed(2)}` : "—", warn: false, bar: c.cpc, max: 5, color: "#8b5cf6" },
                        { label: "FREC.", val: c.frecuencia ? c.frecuencia.toFixed(1) : "—", warn: c.frecuencia > 4.5, bar: c.frecuencia, max: 10, color: c.frecuencia > 4.5 ? "#ef4444" : "#22c55e" },
                        { label: "GASTO", val: c.gasto ? `$${c.gasto.toFixed(0)}` : "—", warn: false, bar: c.gasto, max: totalGasto || 1, color: "#f59e0b" },
                        { label: "CONV.", val: c.conversiones || "—", warn: false, bar: c.conversiones, max: totalConv || 1, color: "#22c55e" },
                      ].map((m, j) => (
                        <div key={j}>
                          <div style={{ fontSize: 9, color: "#374151", letterSpacing: 2, marginBottom: 4 }}>{m.label}</div>
                          <div className="metric-val" style={{ fontSize: 20, color: m.warn ? "#ef4444" : "#e2e8f0" }}>{m.val}</div>
                  ...
                  <div style={{ display: "flex", gap: 10, flexWrap: "wrap" }}>
                    {c.frecuencia > 4.5 && (
                      <div style={{ background: "#1c0a0a", border: "1px solid #7f1d1d", borderRadius: 8, padding: "8px 14px" }}>
                        ...
                        <div className="metric-val" style={{ fontSize: 22, color: "#ef4444" }}>{c.frecuencia.toFixed(1)}</div>
                    ...
                    {c.cpl > 1.5 && (
                      <div style={{ background: "#1c0a0a", border: "1px solid #7f1d1d", borderRadius: 8, padding: "8px 14px" }}>
                        ...
                        <div className="metric-val" style={{ fontSize: 22, color: "#ef4444" }}>${c.cpl.toFixed(2)}</div>
                    ...
                    {c.ctr < 1 && (
                      <div style={{ background: "#1c0911", border: "1px solid #78350f", borderRadius: 8, padding: "8px 14px" }}>
                        ...
                        <div className="metric-val" style={{ fontSize: 22, color: "#f59e0b" }}>{c.ctr?.toFixed(2)}%</div>
